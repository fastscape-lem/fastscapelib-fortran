...
call FastScape_Set_Erosional_Parameters (kf1,kf2,m,n,kd1,kd2,g1,g2,p_flow_dir_exp)
...
z1 = 1.d3
z2 = 1.d3
call FastScape_Set_Marine_Parameters (sealevel, poro1, poro2, z1, z2, ratio, L, kds1, kds2)
...
call cpu_time (time_in)
  ...
  call FastScape_Get_Step (istep)
    ...
    call FastScape_Copy_Total_Erosion (etot)
    call FastScape_Copy_Erosion_Rate (erate)
    call FastScape_Copy_Drainage_Area (a)
    call FastScape_Copy_Chi (chi)
    call FastScape_Copy_Catchment (catchment)
    ...
    field(:,1)=etot
    field(:,2)=erate
    call FastScape_VTK (chi, 2.d0) ! if value > 0, elevation plus value is written to file. If value <0, basement and sealevel + value is written to file.
    call FastScape_VTK (erate, -2.d0) ! if value > 0, elevation plus value is written to file. If value <0, basement and sealevel + value is written to file.
    !field(:,3)=sedflux
